...
需要授权的操作会在终端底部贴底显示一条全宽面板（不是居中的悬浮窗），与聊天内容用一条细分隔线分开；标题标明工具与将要执行的 shell，正文原文显示命令或参数，默认选中「允许本次操作」。
...
面板使用终端自身的深色背景，正文为中灰（`muted`），分隔线更暗（`borderMuted`），选中项靠高亮底色与 `›` 标记而不是彩色文字；金色 `warning` 保留给底栏的上下文告警，`accent` 保留给链接。
...
设置后扩展会切换到 zsh 并同步启动它执行命令，授权面板标题会显示 `需要用户授权 · bash（/usr/bin/zsh）`——`bash` 是 pi 固定使用的工具名，括号里才是实际执行的 shell；未设置时显示 `（bash）`。zsh 比 bash 多一类 `=命令` 展开（`=ls` → `/usr/bin/ls`），会被单引号阻止，因此这种写法改为请求授权；源码里已经用引号或反斜杠保护的写法（`'=ls'`、`"=ls"`、`\=ls`）在 zsh 下本来就是字面量，照常自动执行。`~+` / `~-` 这类目录栈路径也会询问。未设置 `shellPath` 时行为与之前一致。
